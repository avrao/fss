...
<cfajaxproxy cfc="bv" jsclassname="TDF">
<cfajaximport>
...
<head>
<meta charset="utf-8">
<title>Bundle Processing Vis</title></title>
...
var loadInit = 12; //set to number of charts
...
var AreaLoader = loader2({width: 175, height: 175, container: "#AreaNested", id: "AreaLoader",msg:""});
var DistrictLoader = loader2({width: 175, height: 175, container: "#DistrictNested", id: "DistrictLoader",msg:""});
...
if (scheck)
	...
	svg.append("text")
		...
		.attr("x", w/2)
		.attr("y", h - 12)
		.text('Non-nested Bundles.').style('font-size','10px');
	...
	svg.append("text")
		...
		.attr("x", w/2)
		.attr("y", padding/2)
		.text("% FS Bundles Nested ").style('font-size','10px')
		.append("tspan")
		.attr("id","gphName");
...
tle.append("tspan").text(function(d) {return 'Non-nested FS Bundles: ' + comF(d[2]);})
...
tle.selectAll('tspan').remove();	
tle.append("tspan").text(function(d) {return 'Non-nested Bundles: '+comF(d[2]);})
tle.append("tspan").text(function(d) {return '\nTotal Bundles: '+comF(d[4]);})
tle.append("tspan").text(function(d) {return '\nPercent: '+perF(d[3]);})	
...
if(!scheck)
	...
	.append('title')
	.text(function(d,i) {return 'Non-nested Bundles: '+comF(res[i][2]);});
...
zoomed = (function(AR,pID,cID,chID,title,LID,ttl,rng,zm)
	{
	return function zoomed()
		{
		if (loadCnt > 0)
...
if (scheck)
	...
	svg.select('g')
		.call(zoom);
	svg.select('#magnifier')
		.on('click', (function(AR,pID,cID,chID,title,LID,ttl,rng)
			{
			return function doSe()
				{
				d3.select('#searchBlock').style('display','').style('top', window.innerHeight / 3 + window.pageYOffset + 'px');
...
function AreaNested(res)
{	
var rp = res.pop()[0];  
res.shift();
chartHistory[rp].AreaNested = (function(data,v){
	return function(){
	d3.select('#selArea').node().value=v;
	streamLineChart(data,'selArea','AreaNested','svgArea','Area','loader');
	}
...
streamLineChart(res, 'selArea', 'AreaNested', 'svgArea', 'Area', 'loader');
}

function AreaAFSM(res)
{
var rp = res.pop()[0];  
res.shift();
chartHistory[rp].AreaAFSM = (function(data,v){
	return function(){
	d3.select('#selArea').node().value=v;
	rawChart(data,'selArea','AreaAFSM','svgAreaAFSM','Area','loader','Bundles Processed on AFSM/FSS');
	}
})(res, d3.select('#selArea').node().value)
...
rawChart(res, 'selArea', 'AreaAFSM', 'svgAreaAFSM', 'Area', 'loader', 'Bundles Processed on AFSM/FSS');
}

function Area99P(res)
{
var rp = res.pop()[0];  
res.shift();
chartHistory[rp].Area99P = (function(data,v){
	return function(){
	d3.select('#selArea').node().value=v;
	pctChart(data,'selArea','Area99P','svgArea99P','Area','loader','% 99P Loaded');
	}
})(res, d3.select('#selArea').node().value)
...
pctChart(res, 'selArea', 'Area99P', 'svgArea99P', 'Area', 'loader', '% 99P Loaded');
}

function AreaNotLoaded(res)
{
var rp = res.pop()[0];  
res.shift();
chartHistory[rp].AreaNotLoaded = (function(data,v){
	return function(){
	d3.select('#selArea').node().value=v;
	rawChart(data,'selArea','AreaNotLoaded','svgAreaNotLoaded','Area','loader','Bundles Not Loaded (on-Hand)');
	}
})(res, d3.select('#selArea').node().value)
...
rawChart(res, 'selArea', 'AreaNotLoaded', 'svgAreaNotLoaded', 'Area', 'loader', 'Bundles Not Loaded (on-Hand)');
}

function DistrictNested(res)
{
var rp = res.pop()[0];  
res.shift();
//res = res.reverse();
chartHistory[rp].DistrictNested = (function(data,v)
	{
	return function()
		{
		d3.select('#selDistrict').node().value = v;
		streamLineChart(data,'selDistrict','DistrictNested','svgDistrict','District','loader');		
		}
	}
...
streamLineChart(res,'selDistrict','DistrictNested','svgDistrict','District','loader');		
}

function DistrictAFSM(res)
{
var rp = res.pop()[0];  
res.shift();
chartHistory[rp].DistrictAFSM = (function(data,v){
	return function(){
	d3.select('#selDistrict').node().value=v;
	rawChart(data,'selDistrict','DistrictAFSM','svgDistAFSM','District','loader','Bundles Processed on AFSM/FSS');
	}
})(res, d3.select('#selDistrict').node().value)
...
rawChart(res,'selDistrict','DistrictAFSM','svgDistAFSM','District','loader','Bundles Processed on AFSM/FSS');
}

function District99P(res)
{
var rp = res.pop()[0];  
res.shift();
chartHistory[rp].District99P = (function(data,v){
	return function(){
	d3.select('#selDistrict').node().value=v;
	pctChart(data,'selDistrict','District99P','svgDist99P','District','loader','% 99P Loaded');
	}
})(res, d3.select('#selDistrict').node().value)
...
pctChart(res,'selDistrict','District99P','svgDist99P','District','loader','% 99P Loaded');
}

function DistrictNotLoaded(res)
{
var rp = res.pop()[0];  
res.shift();
chartHistory[rp].DistrictNotLoaded = (function(data,v){
	return function(){
	d3.select('#selDistrict').node().value=v;
	rawChart(data,'selDistrict','DistrictNotLoaded','svgDistNotLoaded','District','loader','Bundles Not Loaded (on-Hand)');
	}
})(res, d3.select('#selDistrict').node().value)
...
rawChart(res,'selDistrict','DistrictNotLoaded','svgDistNotLoaded','District','loader','Bundles Not Loaded (on-Hand)');
}

function FacilityNested(res)
{	
var rp = res.pop()[0];  
res.shift();
//res = res.reverse();
chartHistory[rp].FacilityNested = (function(data,v)
	{
	return function()
		{
		d3.select('#selFacility').node().value = v;
		streamLineChart(res,'selFacility','FacilityNested','svgFacility','Facility','loader');		
		}
	}
...
function FacilityAFSM(res)
{	
var rp = res.pop()[0];  
res.shift();
//res = res.reverse();
chartHistory[rp].FacilityAFSM = (function(data,v)
	{
	return function()
		{
		d3.select('#selFacility').node().value = v;
		rawChart(res,'selFacility','FacilityAFSM','svgFacAFSM','Facility','loader','Bundles Processed on AFSM/FSS');		
		}
	}
) (res, d3.select('#selFacility').node().value);
...
function Facility99P(res)
{	
var rp = res.pop()[0];  
res.shift();
//res = res.reverse();
chartHistory[rp].Facility99P = (function(data,v)
	{
	return function()
		{
		d3.select('#selFacility').node().value = v;
		pctChart(res,'selFacility','FacilityAFSM','svgFacAFSM','Facility','loader','% 99P Loaded');		
		}
	}
) (res, d3.select('#selFacility').node().value);
...
pctChart(res,'selFacility','Facility99P','svgFac99P','Facility','loader','% 99P Loaded');		
}

function FacilityNotLoaded(res)
{	
var rp = res.pop()[0];  
res.shift();
//res = res.reverse();
chartHistory[rp].FacilityNotLoaded = (function(data,v)
	{
	return function()
		{
		d3.select('#selFacility').node().value = v;
		rawChart(res,'selFacility','FacilityNotLoaded','svgFacNotLoaded','Facility','loader','Bundles Not Loaded (on-Hand)');		
		}
	}
) (res, d3.select('#selFacility').node().value);
...
var ssv = document.getElementById('selSV').value;
var sfss = document.getElementById('selFSS').value;

var e=new TDF();
e.setCallbackHandler(Display_Overalls);	
e.setErrorHandler(area_data_err);
e.getOverallPlant(ReqId,'<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, sarea, sdist, sfac);
for (var i=0; i < 8; i++)
	{
	...
	e.setErrorHandler(area_data_err);
	e.getPlantTrend(ReqId,'<cfoutput>#ivdata#</cfoutput>', trendDate, trendEnd, newrange, sarea, sdist, sfac, sfss, ssv);
	}

var e=new TDF();
e.setCallbackHandler(AreaNested);	
e.setErrorHandler(area_data_err);
e.getByAreaPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '1');

var e=new TDF();
e.setCallbackHandler(DistrictNested);	
e.setErrorHandler(area_data_err);
e.getByDistrictPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '1', sarea);

var e=new TDF();
e.setCallbackHandler(FacilityNested);	
e.setErrorHandler(area_data_err);
e.getByFacilityPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '1', sarea, sdist);

var e=new TDF();
e.setCallbackHandler(AreaAFSM);	
e.setErrorHandler(area_data_err);
e.getByAreaPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '2');

var e=new TDF();
e.setCallbackHandler(DistrictAFSM);	
e.setErrorHandler(area_data_err);
e.getByDistrictPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '2', sarea);

var e=new TDF();
e.setCallbackHandler(FacilityAFSM);	
e.setErrorHandler(area_data_err);
e.getByFacilityPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '2', sarea, sdist);

var e=new TDF();
e.setCallbackHandler(Area99P);	
e.setErrorHandler(area_data_err);
e.getByAreaPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '3');

var e=new TDF();
e.setCallbackHandler(District99P);	
e.setErrorHandler(area_data_err);
e.getByDistrictPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '3', sarea);

var e=new TDF();
e.setCallbackHandler(Facility99P);	
e.setErrorHandler(area_data_err);
e.getByFacilityPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '3', sarea, sdist);

var e=new TDF();
e.setCallbackHandler(AreaNotLoaded);	
e.setErrorHandler(area_data_err);
e.getByAreaPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '4');

var e=new TDF();
e.setCallbackHandler(DistrictNotLoaded);	
e.setErrorHandler(area_data_err);
e.getByDistrictPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '4', sarea);

var e=new TDF();
e.setCallbackHandler(FacilityNotLoaded);	
e.setErrorHandler(area_data_err);
e.getByFacilityPlant(ReqId, '<cfoutput>#ivdata#</cfoutput>', sfss, ssv, sd, sdend, '4', sarea, sdist);
...
if (smlr != '' &&  smlr.indexOf(',') == -1)
	...
	e.setErrorHandler(area_data_err);
	e.getcsa(ReqId,'<cfoutput>#ivdata#</cfoutput>',smlr);
	}
...
for (var i=0; i < 8; i++)
	...
	e.setCallbackHandler(trend_Chart);	
	e.setErrorHandler(area_data_err);
	e.getByTrend(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,trendDate,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,newrange,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);
	}

var e=new TDF();
e.setCallbackHandler(Mailer_Chart);	
e.setErrorHandler(area_data_err);
e.getByMailerData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd,smflag);
if (sdir == 'O')
	{
	var e=new TDF();
	e.setCallbackHandler(zip3_Chart);	
	e.setErrorHandler(area_data_err);
	e.getByDestZip3(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);
	}
else
	{
	var e=new TDF();
	...
	e.setErrorHandler(area_data_err);
	e.getByOrgFacData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);
	}

var e=new TDF();
e.setCallbackHandler(state_Chart);	
e.setErrorHandler(area_data_err);
e.getByState(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(Display_Overalls);	
e.setErrorHandler(area_data_err);
e.getByOverallData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(Area_Chart);	
e.setErrorHandler(area_data_err);
e.getByAreaData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(District_Chart);	
e.setErrorHandler(area_data_err);
e.getByDistData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(Facility_Chart);	
e.setErrorHandler(area_data_err);
e.getByFacilityData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);
<!---		var e=new TDF();
e.setCallbackHandler(DOW_Chart);	
e.setErrorHandler(area_data_err);
e.getByDayData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,"","","","Y",spol,shyb,'',sDOW,sfac);--->

var e=new TDF();
e.setCallbackHandler(Cntr_Chart);	
e.setErrorHandler(area_data_err);
e.getByCntrData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(Class_Chart);	
e.setErrorHandler(area_data_err);
e.getByClassData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(Cat_Chart);	
e.setErrorHandler(area_data_err);
e.getByCatData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(EOD_Chart);	
e.setErrorHandler(area_data_err);
e.getByEODData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(SVCSTD_Chart);	
e.setErrorHandler(area_data_err);
e.getBySvcStdData(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(Air_Chart);	
e.setErrorHandler(area_data_err);
e.getByAir(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(FSS_Chart);	
e.setErrorHandler(area_data_err);
e.getByFSS(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(pol_Chart);	
e.setErrorHandler(area_data_err);
e.getByPolitical(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(enttype_Chart);	
e.setErrorHandler(area_data_err);
e.getByEntType(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(induct_Chart);	
e.setErrorHandler(area_data_err);
e.getByInductMthd(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(fullsvc_Chart);	
e.setErrorHandler(area_data_err);
e.getByFuLLSvc(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(ur_Chart);	
e.setErrorHandler(area_data_err);
e.getByURT(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);

var e=new TDF();
e.setCallbackHandler(urbk_Chart);	
e.setErrorHandler(area_data_err);
e.getByURBK(ReqId,'<cfoutput>#ivdata#</cfoutput>',srlupv,sdir,seod,sd,sdend,sarea,sdist,sclass,scat,sfss,smode,sair,scntr,scert,spol,shyb,'',sDOW,sfac,srng,smlr,szip3,sorg,seodt,sindc,state,sur,surbko,surbkd);
}
--->
...
var comF = d3.format(',');
var cScale2 = d3.scale.linear().domain([0,0.699,0.7,0.799,0.8,0.899,0.9,0.949,0.95,1]).range(["#EC9792","#EC9792","#FDBB9B","#FDBB9B","#fee090","#fee090","#91bfdb","#91bfdb","#7C9FCE","#7C9FCE"]);	
var idx = aryCol.indexOf('FS_BNDL_NESTED_PCT');
var toColor = cScale2(res[0][idx]);
d3.select('#pctFSNested').style('background',toColor).text(perF(res[0][idx]));
idx = aryCol.indexOf('TOTAL_BNDL');
d3.select('#TotalBundles').style('background',toColor).text(comF(res[0][idx]));

idx = aryCol.indexOf('FS_BNDL_PROC');
d3.select('#FSProcessed').style('background',toColor).text(comF(res[0][idx]));

idx = aryCol.indexOf('FS_BNDL_NEST');
d3.select('#FSNested').style('background',toColor).text(comF(res[0][idx]));

idx = aryCol.indexOf('FS_AFSMFS_PROCSS_BNDL');
d3.select('#FS-AFMS-FSS').style('background',toColor).text(comF(res[0][idx]));

idx = aryCol.indexOf('NNP_CONT');
d3.select('#Nested99P').style('background',toColor).text(comF(res[0][idx]));

idx = aryCol.indexOf('NNH_CONT');
d3.select('#Nested99H').style('background',toColor).text(comF(res[0][idx]));

idx = aryCol.indexOf('NNP_LOADED');
d3.select('#Loaded99P').style('background',toColor).text(comF(res[0][idx]));

idx = aryCol.indexOf('NNP_NOT_LOADED');
d3.select('#NotLoaded99P').style('background',toColor).text(comF(res[0][idx]));

idx = aryCol.indexOf('LOADED_99P_CONTR_PCT');
d3.select('#pctLoaded99P').style('background',toColor).text(perF(res[0][idx]));

idx = aryCol.indexOf('BNDLS_NOT_LOADED_ONHAND');
d3.select('#NotLoadedBundles').style('background',toColor).text(comF(res[0][idx]));
}
...
var nnIdx = aryCols.indexOf('INTNONNESTED');
var pctIdx = aryCols.indexOf('PCTNESTED');
...
hold.push(filterHds[4]);
hold.push(filterHds[5]);
var hold2 =[];
hold2.push(filterArgs[3]);
hold2.push(filterArgs[4]);
hold2.push(filterArgs[5]);
...
<cfinclude template="jscripts/bv_vis.js">
</script>
</head>
...
<!---input type="hidden" id="selSV" name="selSV" value="">
...
<div id="filtDiv">
	...
	<fieldset class="fset"><legend>FSS Site:</legend>
		<select class ="slct" name="selFSS" id="selFSS" onchange="filterArgs[4]=this.options[this.selectedIndex].text;cfrm();">
		<option value="">Combined</option> 
		<option value="Y">FSS</option> 
		<option value="N">Non-FSS</option> 
		</select> 
	</fieldset>
	<fieldset class="fset"><legend>SV Site:</legend>
		<select class ="slct" name="selSV" id="selSV" onchange="filterArgs[5]=this.options[this.selectedIndex].text;cfrm();">
		<option value="">Combined</option> 
		<option value="Y">SV</option> 
		<option value="N">Non-SV</option> 
		</select> 
	</fieldset>
...
<h2>Bundle Visibility Visualization</h2><iframe id="txtArea1" style="display:none"></iframe> 
</div>
...
<legend style="font-weight:  bold;font-size: 14px; text-align: center; margin: 0px auto;">Total Bundles<br>Processed</legend>
<div id="TotalBundles" class="overall">
</div>
</div>
<div style="margin:0px 3px 0px 3px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: Arial, Helvetica, sans-serif;">
<legend style="font-weight:  bold;font-size: 14px; text-align: center; margin: 0px auto;">FS Bundles<br>Processed</legend>
<div id="FSProcessed" class="overall">
</div>
</div>
<div style="margin:0px 3px 0px 3px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: Arial, Helvetica, sans-serif;">
<legend style="font-weight:  bold;font-size: 14px; text-align: center; margin: 0px auto;">FS Bundles<br>Nested</legend>
<div id="FSNested" class="overall">
</div>
</div>
<div style="margin:0px 3px 0px 3px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: Arial, Helvetica, sans-serif;">
<legend style="font-weight:  bold;font-size: 14px; text-align: center; margin: 0px auto;">FS Nested<br>AFSM/FSS</legend>
<div id="FS-AFMS-FSS" class="overall">
</div>
</div>
<div style="margin:0px 3px 0px 3px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: Arial, Helvetica, sans-serif;">
<legend style="font-weight:  bold;font-size: 14px; text-align: center; margin: 0px auto;">% FS Bundles<br>Nested</legend>
<div id="pctFSNested" class="overall">
</div>
</div>
<div style="margin:0px 3px 0px 3px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: Arial, Helvetica, sans-serif;">
<legend style="font-weight:  bold; font-size: 14px; text-align: center; margin: 0px auto;">Total Nested<br>99P Containers</legend>
<div id="Nested99P" class="overall">
</div>
</div>
<div style="margin:0px 3px 0px 3px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: Arial, Helvetica, sans-serif;">
<legend style="font-weight:  bold; font-size: 14px; text-align: center; margin: 0px auto;">Total Nested<br>99H Containers</legend>
<div id="Nested99H" class="overall">
</div>
</div>
<div style="margin:0px 3px 0px 3px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: Arial, Helvetica, sans-serif;">
<legend style="font-weight:  bold; font-size: 14px; text-align: center; margin: 0px auto;">99P<br>Loaded</legend>
<div id="Loaded99P" class="overall">
</div>
</div>
<div style="margin:0px 3px 0px 3px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: Arial, Helvetica, sans-serif;">
<legend style="font-weight: bold; font-size: 14px; text-align: center; margin: 0px auto;">99P<br>Not Loaded</legend>
<div id="NotLoaded99P" class="overall">
</div>
</div>
<div style="margin:0px 3px 0px 3px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: Arial, Helvetica, sans-serif;">
<legend style="font-weight:  bold; font-size: 14px; text-align: center; margin: 0px auto;">% 99P<br>Loaded</legend>
<div id="pctLoaded99P" class="overall">
</div>
</div>
<div style="margin:0px 3px 0px 3px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: Arial, Helvetica, sans-serif;">
<legend style="font-weight:  bold; font-size: 14px; text-align: center; margin: 0px auto;">Bundles Not<br>Loaded</legend>
<div id="NotLoadedBundles" class="overall">
</div>
</div>
<!---div style="margin:0px 50px 20px 50px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: times, Times New Roman;">
<legend style='font-weight:bold;font-size: 24px;'>Failed Pieces</legend>
<div id="OverallFail" style="font-size: 12px; padding: 8px 0px; border-radius: 15px; box-sizing: border-box; color: white; text-shadow:#474747 2px 2px 4px;height: 30px; width: 100px">
</div>
</div>
<div style="margin:0px 50px 20px 50px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: times, Times New Roman;">
<legend style='font-weight:bold;font-size: 24px;'>Total Pieces</legend>
<div id="OverallPieces" style="font-size: 18px;padding:40px 0px;border-radius: 15px;box-sizing:border-box;color:white;text-shadow:#474747 2px 2px 4px;height:100px;width:200px">
</div>
</div--->
<!---
...
<div id="AreaNested" style="display:inline-block;max-width:380px;"></div>
<div id="DistrictNested" style="display:inline-block;max-width:380px;"></div>
<div id="FacilityNested" style="display:inline-block;max-width:380px;"></div>
<div id="AreaAFSM" style="display:inline-block;max-width:380px;"></div>
<div id="DistrictAFSM" style="display:inline-block;max-width:380px;"></div>
<div id="FacilityAFSM" style="display:inline-block;max-width:380px;"></div>
<div id="Area99P" style="display:inline-block;max-width:380px;"></div>
<div id="District99P" style="display:inline-block;max-width:380px;"></div>
<div id="Facility99P" style="display:inline-block;max-width:380px;"></div>
<div id="AreaNotLoaded" style="display:inline-block;max-width:380px;"></div>
<div id="DistrictNotLoaded" style="display:inline-block;max-width:380px;"></div>
<div id="FacilityNotLoaded" style="display:inline-block;max-width:380px;"></div>
</div>
