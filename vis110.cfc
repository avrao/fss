...
<!---            (select /*+ result_cache */ distinct area as area_id,site_name as area_name,district as district_id,district_name
            ...
            <cfif selPol neq ''>and fp.political_mail_ind in (#Replace(selPol,"''","'","All")#) </cfif>
			<!---<!---<cfif selDOW neq ''>and to_char(actual_dlvry_date,'d') in (#Replace(selDOW,"''","'","All")#)</cfif>--->--->
...
<!---            
			...
			FROM (  SELECT to_char(actual_dlvry_date,'d') as dow, UPPER(to_char(actual_dlvry_date,'day')) as day,
			sum(#piece_cnt#) as tp_tot,
			sum(#fail_piece_cnt#) as fp_tot,
			sum(#piece_cnt#-#fail_piece_cnt#) 
            ...
            <cfif selOrgFac is not ''>and fp.orgn_fac_seq_id in(#Replace(selOrgFac,"''","'","All")#) </cfif>                          
<cfif selCert is not ''>and fp.certified_mailer_ind  in(#Replace(selCert,"''","'","All")#) </cfif>              
			group by to_char(actual_dlvry_date,'d'),UPPER(to_char(actual_dlvry_date,'day'))
			) gp
            where  tp_tot > 0
			order by fp_tot desc
		</cfquery>        
...
<!---        select  distinct area ,site_name as area_name,district,district_name
	...
	<cffunction name="getDates" access="remote" returntype="array">
		...
		<cfelse>
          	<cfquery name="wk" datasource="#dsn#">
               SELECT dt AS beg_date,DT  AS end_date,TO_CHAR (dt, 'MM/DD/YYYY') AS strdate,to_char(DT ,'mm/dd/yyyy') as strend  
                FROM (SELECT   (SELECT MIN (ACTUAL_DLVRY_DATE) FROM V_BI_ADD_OD_RPT_DAY_ZIP3)+ ROWNUM-1 AS dt
                FROM V_BI_ADD_OD_RPT_DAY_ZIP3
                WHERE ROWNUM <=  (SELECT MAX (ACTUAL_DLVRY_DATE) FROM V_BI_ADD_OD_RPT_DAY_ZIP3)- (SELECT MIN (ACTUAL_DLVRY_DATE) FROM V_BI_ADD_OD_RPT_DAY_ZIP3))
                ORDER BY dt DESC            
...
<CFSET seq_list = valuelist(seq_q.crid_seq_id)>
		...
		<cfquery name="dta" datasource="#dsn#">
         ...
         (
			 SELECT 
               ...
               to_char(fp.actual_dlvry_date,'mm/dd/yyyy') as actual_dlvry_date, 
    ...
    from
             IV_app.BI_ADD_FAILED_PIECE_SCAN fp
...
<!---            (select /*+ result_cache */ distinct area as area_id,site_name as area_name,district as district_id,district_name
            ...
            select /*+ opt_param('_GBY_HASH_AGGREGATION_ENABLED' 'true')  parallel(4)   */ fac_name as <cfif selDir is 'O'>ORIGIN_FACILITY <cfelse>DESTN_FACILITY</cfif> , 
...
<!---                    
			...
			SELECT /*+ opt_param('_GBY_HASH_AGGREGATION_ENABLED' 'true')  parallel(4)   */ gp.*
			FROM (  SELECT
                   political_mail_ind  as political,
                   case when political_mail_ind = 'N' then 'NON-POLITICAL' else 'POLITICAL' end as descrip,			
                   sum(#piece_cnt#) as tp_tot,
		   sum(#fail_piece_cnt#) as fp_tot,
                   decode(sum(#piece_cnt#),0,0, 
			sum(#piece_cnt#-#fail_piece_cnt#) 
