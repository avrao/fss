<!doctype html>
<cfajaxproxy cfc="GridReportStdD3" jsclassname="GRS">
<cfajaximport>
...
<head>
<meta charset="utf-8">
<title>Failed Pieces - 3-Digit Listing</title>
...
<script>
	<cfoutput>
	var params=['#selFacility#','#selDOW#','#selEod#','#selDir#','#selHybStd#','#selClass#','#selCategory#','#selFSS#','#selAir#','#selMode#','#rpt_range#','#BG_DATE#','#chkpol#','#selMailer#']
	</cfoutput>
	...
	{	
		...
		var low = d3.min(dataGroup,function(d){return d[4][0];});
	...
	/*	var	cScale = d3.scale.linear().domain([0,1]).range(["#0C0","#0C0"]);	
		...
		container.selectAll(".rectBar2")
			   ...
			   .append("rect")
				.attr("class", "rectBar2")
			   .attr("x", function(d,i) {	return xScale(order[i])+rdiv-rw/2;})
			   .attr("y", function(d) {return yScale2(d[4][0]);})
			   .attr("width", rw)
			   .attr("height", function(d) {return Math.abs(yScale2(d[4][0])-yScale2(low));})
			   .attr("fill",'#A00')
			   .append('title')
			   .text(function(d) {return 'Failed Pieces: '+comF(d[4][0]);})
	...
	function init()
	{
		...
		e.setCallbackHandler(makeTable);	
		e.setErrorHandler(data_err);
		<cfoutput>e.getMainData('#opsvisdata#','#selDir#','#selEod#','#BG_DATE#','#ED_DATE#','#selClass#','#selCategory#','#selFSS#','#selMode#','#selAir#',"","Y",'#chkpol#','#selHybStd#','','#selFacility#','#selDOW#','','','#selMailer#');</cfoutput>
	...
	function cfrm_sub()
	{
		...
		e.setCallbackHandler(makeTable);	
		e.setErrorHandler(data_err);
		e.getMainData('<cfoutput>#opsvisdata#</cfoutput>',sdir,seod,sd,sdend,sclass,scat,sfss,smode,sair,"","Y",spol,shyb,'',params[0],sDOW,'','',params[13]);
	}
...
*/
	...
	/*	window.open('DiagReportD3.cfm?WKN='+sd+'&BG_DATE=' +sd+"&ED_DATE="+sdend+"&RPT_RANGE="+rng+"&SELAREA="+sarea+"&SELDISTRICT="+sdist
		...
		*/
		...
		lnkForm.append("input").attr("type","hidden").attr("name","selOrg3").attr("value",N[1]);
		...
		lnkForm.append("input").attr("type","hidden").attr("name","selDest3").attr("value",N[2]);
		lnkForm.append("input").attr("type","hidden").attr("name","selPhysCntr").attr("value",'');
		lnkForm.append("input").attr("type","hidden").attr("name","selDestFac").attr("value",N[3]);
	...
	function makeTable(res)
	{
		...
		data.forEach(function(d,i){
			res[i]=[];
			count=0;
			res[i][count++]=d.ORGN_FAC_ZIP_3;				
			res[i][count++]=d.ORG_FAC_NAME;
			res[i][count++]=d.DESTN_FAC_ZIP_3;
			res[i][count++]=d.DEST_FAC_NAME;
			res[i][count++]=[d.FAILED_PIECES,d.ORGN_FAC_ZIP_3,d.DESTN_FAC_ZIP_3,d.DESTN_FAC_SEQ_ID];
			res[i][count++]=d.MAILED_PIECES;
			res[i][count++]=d.ON_TIME_PER;
		});
		
		BarChart(res);
		
		h1row=['Origin 3-Digit','Origin Facility','Dest 3-Digit','Dest Facility','Failed','Total','On-Time %'];
		...
		.append('td')
		.style('padding','3px')
		.style('text-decoration',function(d,i){if(i==4)return 'underline';})
		.style('color',function(d,i){if(i==4)return 'blue';})
		.style('cursor',function(d,i){if(i==4)return 'pointer';})
		.on('click',function(d,i){
			if(i==4)Diag_Report_Lnk(d);
		})
		.style('text-align',function(d,i) {return (i<4&&i%2==1) ? 'Left' : 'Center'})
		.text(function(d,i){if(i >3 && i <6){if(i==4)return (d[0]>=100)?siF(d[0]):d[0];return (d>=100)?siF(d):d;}if(i>5)return perF(d);return d;});
...
<h2>IMb - FAILED PIECES THREE DIGIT ZIP - MATRIX</h2><iframe id="txtArea1" style="display:none"></iframe>
