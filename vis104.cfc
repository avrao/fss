	...
	<cffunction name="getDates" access="remote" returntype="array">
		...
		<cfif range is 'MON'>
			<cfquery name="wk" datasource="#dsn#">
				...
				FROM
				(select (select min(START_THE_CLOCK_DATE) from iv_app.BI_AGG_STC_SPD_OD_DAY)+rownum as dt
				from iv_app.BI_AGG_STC_SPD_OD_DAY
				where rownum <= (select max(START_THE_CLOCK_DATE) from iv_app.BI_AGG_STC_SPD_OD_DAY)-(select min(START_THE_CLOCK_DATE) from iv_app.BI_AGG_STC_SPD_OD_DAY))
		...
		<cfelse>
			<cfquery name="wk" datasource="#dsn#">
		         SELECT dt AS beg_wk_date,
                DT + 6 AS end_wk_date,
                TO_CHAR (dt, 'MM/DD/YYYY') AS strdate
                FROM (SELECT   (SELECT MIN (START_THE_CLOCK_DATE) FROM iv_app.BI_AGG_STC_SPD_OD_DAY)
                + ROWNUM
                AS dt
                FROM iv_app.BI_AGG_STC_SPD_OD_DAY
                WHERE ROWNUM <=
                (SELECT MAX (START_THE_CLOCK_DATE)
                FROM iv_app.BI_AGG_STC_SPD_OD_DAY)
                - (SELECT MIN (START_THE_CLOCK_DATE)
                FROM iv_app.BI_AGG_STC_SPD_OD_DAY))
