<!doctype html>
<cfajaxproxy cfc="visBBsnapshot" jsclassname="TDF">
<cfajaximport>

<cfsetting showdebugoutput="no">
<cfset datasource = "iv_spduser">
<html>
<head>
<meta charset="utf-8">
<title>Bundle Breakout [DEV]</title>
...
<script>
	...
	{	
		...
		if(scheck)
		{
			...
			.on('click',(function(AR,pID,cID,chID,title){
				return function shwTbl(){
					...
					{
						'maxRows':20,
						'headers':['ID','Name','Broken Bundles','Percent','Total'],
		...
		var tle=fpie.enter()
			   ...
			   .on('mouseover',function(d,i){
					...
					.duration(200)		
					.style("opacity", 1);
					tooldiv.html( 'Broken Bundles: '+comF(d[2]) + "<br/>"  + 'Total Bundles: '+comF(d[4]) + "<br/>"  +'Percent: '+perF(d[3]))	
					.style("left", (d3.event.pageX) + "px")		
					.style("top", (d3.event.pageY +20) + "px");	
			   })
		...
		/*tle.append("tspan").attr('display','block').text(function(d) {return 'Broken Bundles: '+comF(d[2]);})
		...
		if(!scheck)
		{
			...
			tle=svg.append("g")
			   ...
			   .on('mouseover',function(d,i){d3.select(this).attr('opacity',0.5).style('cursor','pointer');d3.selectAll('#'+chartId+' .failDropBars').filter(".r"+i).attr('opacity',1).attr('stroke','cyan');
					...
					.duration(200)		
					.style("opacity", 1);
					tooldiv.html( 'Broken Bundles: '+comF(res[i][2]) + "<br/>"  + 'Total Bundles: '+comF(res[i][4]) + "<br/>"  +'Percent: '+perF(res[i][3]))	
					.style("left", (d3.event.pageX) + "px")		
					.style("top", (d3.event.pageY +20) + "px");	})
			...
			tle.append("tspan").attr('display','block').text(function(d,i) {return 'Broken Bundles: '+comF(res[i][2]);})
			tle.append("tspan").attr('display','block').text(function(d,i) {return '\nTotal Bundles: '+comF(res[i][4]);})
	...
	function FSFacility_Chart(res)
	{
		var resId = res.pop()[0];
			res.shift();
			res=res.reverse();	
			chartHistory[resId].FSFacility_Chart=(function(data){
				return function(){
					streamLineChart(data,'selFSFacility','chart-FSFacility','svgFSFacility','First Scan Facility','loader');
				}
			})(res)
		if(ReqId != resId)
		{
		...
		streamLineChart(res,'selFSFacility','chart-FSFacility','svgFSFacility','First Scan Facility','loader');
	}
	
	function Device_Chart(res)
	{
		var resId = res.pop()[0];
			res.shift();
			res=res.reverse();	
			chartHistory[resId].Device_Chart=(function(data){
				return function(){
					streamLineChart(data,'selDevice','chart-Device','svgDevice','First Scan Equipment ','loader');
				}
			})(res)
		if(ReqId != resId)
		{
		...
		streamLineChart(res,'selDevice','chart-Device','svgDevice','First Scan Equipment ','loader');
	}
	
	function MCLASS_Chart(res)
	{
		var resId = res.pop()[0];
			res.shift();
			res=res.reverse();	
			chartHistory[resId].MCLASS_Chart=(function(data){
				return function(){
					streamLineChart(data,'selMClass','chart-MClass','svgMCLASS','Mail Class','loader');
				}
			})(res)
		if(ReqId != resId)
		{
		...
		streamLineChart(res,'selMClass','chart-MClass','svgMCLASS','Mail Class','loader');
	}
	
	function PKG_Chart(res)
	{
		var resId = res.pop()[0];
			res.shift();
			res=res.reverse();	
			chartHistory[resId].PKG_Chart=(function(data){
				return function(){
					streamLineChart(data,'selPKG','chart-PKG','svgPKG','Bundle Sort Level ','loader');
				}
			})(res)
		if(ReqId != resId)
		{
		...
		streamLineChart(res,'selPKG','chart-PKG','svgPKG','Bundle Sort Level ','loader');
	}
	
	function Disc_Chart(res)
	{
		var resId = res.pop()[0];
			res.shift();
			res=res.reverse();	
			chartHistory[resId].Disc_Chart=(function(data){
				return function(){
					streamLineChart(data,'selDIS','chart-DIS','svgDIS','Entry Discount','loader');
				}
			})(res)
		if(ReqId != resId)
		{
		...
		streamLineChart(res,'selDIS','chart-DIS','svgDIS','Entry Discount','loader');
	}
	
	function Prep_Chart(res)
	{
		var resId = res.pop()[0];
			res.shift();
			res=res.reverse();	
			chartHistory[resId].Prep_Chart=(function(data){
				return function(){
					streamLineChart(data,'selPREP','chart-PREP','svgPrep','Mail Preparer','loader');
				}
			})(res)
		if(ReqId != resId)
		{
		...
		streamLineChart(res,'selPREP','chart-PREP','svgPrep','Mail Preparer','loader');
	}
	
	function Own_Chart(res)
	{
		var resId = res.pop()[0];
			res.shift();
			res=res.reverse();	
			chartHistory[resId].Own_Chart=(function(data){
				return function(){
					streamLineChart(data,'selOWN','chart-OWN','svgOWN','Mail Owner','loader');
				}
			})(res)
		if(ReqId != resId)
		{
		...
		streamLineChart(res,'selOWN','chart-OWN','svgOWN','Mail Owner','loader');
	}
	
	function MH_Chart(res)
	{
		var resId = res.pop()[0];
			res.shift();
			res=res.reverse();	
			chartHistory[resId].MH_Chart=(function(data){
				return function(){
					streamLineChart(data,'selMH','chart-MH','svgMH','Multiple Handlings','loader');
				}
			})(res)
		if(ReqId != resId)
		{
		...
		streamLineChart(res,'selMH','chart-MH','svgMH','Multiple Handlings','loader');
	}
	...
	function cfrm_sub()
	{
		...
		var sdst = document.getElementById("selDistrict").value;
		var sf = document.getElementById("selFacility").value;
		var sfsf = document.getElementById("selFSFacility").value;
		var sdev = document.getElementById("selDevice").value;
		var smclass = document.getElementById("selMClass").value;
		var spkg = document.getElementById("selPKG").value;
		var sdis = document.getElementById("selDIS").value;
		var sprep = document.getElementById("selPREP").value;
		var sown = document.getElementById("selOWN").value;
		var smh = document.getElementById("selMH").value;
		var sbrk = d3.select('#selBRKDWN:checked').node().value;//document.getElementById("selBRKDWN").value;
		ReqId++;
		...
		e.setErrorHandler(area_data_err);
		e.getByOverallData(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,sa,sdst,sf,sfsf,sdev,smclass,spkg,sdis,sprep,sown,smh,sbrk);
		var e=new TDF();
		e.setCallbackHandler(Area_Chart);	
		e.setErrorHandler(area_data_err);
		e.getByAreaData(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,'',sdst,sf,sfsf,sdev,smclass,spkg,sdis,sprep,sown,smh,sbrk);
		var e=new TDF();
		e.setCallbackHandler(District_Chart);	
		e.setErrorHandler(area_data_err);
		e.getByDistData(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,sa,'',sf,sfsf,sdev,smclass,spkg,sdis,sprep,sown,smh,sbrk);
		var e=new TDF();
		e.setCallbackHandler(Facility_Chart);	
		e.setErrorHandler(area_data_err);
		e.getByFacilityData(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,sa,sdst,'',sfsf,sdev,smclass,spkg,sdis,sprep,sown,smh,sbrk);
		var e=new TDF();
		e.setCallbackHandler(FSFacility_Chart);	
		e.setErrorHandler(area_data_err);
		e.getByFirstScanFacility(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,sa,sdst,sf,'',sdev,smclass,spkg,sdis,sprep,sown,smh,sbrk);
		var e=new TDF();
		e.setCallbackHandler(Device_Chart);	
		e.setErrorHandler(area_data_err);
		e.getByDevice(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,sa,sdst,sf,sfsf,'',smclass,spkg,sdis,sprep,sown,smh,sbrk);
		var e=new TDF();
		e.setCallbackHandler(MCLASS_Chart);	
		e.setErrorHandler(area_data_err);
		e.getByMClassData(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,sa,sdst,sf,sfsf,sdev,'',spkg,sdis,sprep,sown,smh,sbrk);
		var e=new TDF();
		e.setCallbackHandler(PKG_Chart);	
		e.setErrorHandler(area_data_err);
		e.getByPKGData(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,sa,sdst,sf,sfsf,sdev,smclass,'',sdis,sprep,sown,smh,sbrk);
		var e=new TDF();
		e.setCallbackHandler(Disc_Chart);	
		e.setErrorHandler(area_data_err);
		e.getByDiscData(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,sa,sdst,sf,sfsf,sdev,smclass,spkg,'',sprep,sown,smh,sbrk);
		var e=new TDF();
		e.setCallbackHandler(Prep_Chart);	
		e.setErrorHandler(area_data_err);
		e.getByPreparerData(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,sa,sdst,sf,sfsf,sdev,smclass,spkg,sdis,'',sown,smh,sbrk);
		var e=new TDF();
		e.setCallbackHandler(Own_Chart);	
		e.setErrorHandler(area_data_err);
		e.getByOwnerData(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,sa,sdst,sf,sfsf,sdev,smclass,spkg,sdis,sprep,'',smh,sbrk);
		var e=new TDF();
		e.setCallbackHandler(MH_Chart);	
		e.setErrorHandler(area_data_err);
		e.getByMH(ReqId,'<cfoutput>#datasource#</cfoutput>',sd,sdend,sa,sdst,sf,sfsf,sdev,smclass,spkg,sdis,sprep,'',smh,sbrk);
	...
	{	
		...
		if(svg.node())
		{
			...
			fpie.enter()
				   ...
				   .text(function(d) {return 'Broken Bundles: '+comF(d[2]);})
			...
			fpie.transition().duration(500).attr("x", function(d,i) { return xScale(new Date(d[5]))-rw/2+2;})
				   ...
				   .select('title').text(function(d) {return 'Broken Bundles: '+comF(d[2]);});
	...
	function HCLpicker(eve,sColor,t)
	{
		...
		d3.select('#resetColor').on('click',function(){
			if(t.name == 'c0070')
				HCLcolor = c0070="#EC9792";
			else if(t.name == 'c7080')
				HCLcolor = c7080="#FDBB9B";
			else if(t.name == 'c8090')
				HCLcolor = c8090="#fee090";
			else if(t.name == 'c9095')
				HCLcolor = c9095="#91bfdb";
			else if(t.name == 'c9510')
				HCLcolor = c9510="#7C9FCE";
			t.value = HCLcolor;
			t.style.background=HCLcolor;	
			t.parentNode.style.background=HCLcolor;
			d3.select("#hclCont").style("display","none");
...
<div id='filtDiv'>
	<fieldset class="" style='display:none;' ><legend>Report For:</legend>
		...
		<input type='hidden' id="selFSFacility" name='selFSFacility' value=''>
		<input type='hidden' id="selDevice" name='selDevice' value=''>
		<input type='hidden' id="selMClass" name='selMClass' value=''>
		<input type='hidden' id="selPKG" name='selPKG' value=''>
		<input type='hidden' id="selDIS" name='selDIS' value=''>
		<input type='hidden' id="selPREP" name='selPREP' value=''>
		<input type='hidden' id="selOWN" name='selOWN' value=''>
		<input type='hidden' id="selMH" name='selMH' value=''>
	</fieldset>
...
% Broken: (2 Scan Logic)-<input type='radio' class ="slctSmall" id='selBRKDWN' name='selBRKDWN' onchange = "chartHistory.length=ReqId+1;chartHistory[ReqId+1]={};var ht=d3.select('#histTrack').node();ht.options.length=ReqId+1;ht.options[ReqId+1]=new Option('Broken '+(this.value-1),ReqId+1);ht.selectedIndex=ReqId+1;cfrm_sub();" value = 2 checked> 
(3 Scan Logic)-<input  type='radio' class ="slctSmall" id='selBRKDWN' name='selBRKDWN' onchange = "chartHistory.length=ReqId+1;chartHistory[ReqId+1]={};var ht=d3.select('#histTrack').node();ht.options.length=ReqId+1;ht.options[ReqId+1]=new Option('Broken '+(this.value-1),ReqId+1);ht.selectedIndex=ReqId+1;cfrm_sub();" value = 3>
</label>
...
<h2>Bundle Breakout [DEV]</h2><iframe id="txtArea1" style="display:none"></iframe>
...
<div style="margin:15px 50px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: times, Times New Roman;vertical-align:top;"><legend style='font-weight:bold;font-size: 24px;'>% Unbroken Bundles</legend><div class='ovAll' id="OverallScore" style="font-size: 18px;padding:40px 0px;border-radius: 15px;box-sizing:border-box;color:white;text-shadow:#474747 2px 2px 4px;height:100px;width:200px"></div></div>
<div style="margin:15px 50px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: times, Times New Roman;vertical-align:top;"><legend style='font-weight:bold;font-size: 24px;'>Broken Bundles</legend><div class='ovAll' id="OverallFail" style="font-size: 18px;padding:40px 0px;border-radius: 15px;box-sizing:border-box;color:white;text-shadow:#474747 2px 2px 4px;height:100px;width:200px"></div></div>
<div style="margin:15px 50px;display:inline-block;max-width:380px;text-align:center;text-variant: small-caps;font-family: times, Times New Roman;vertical-align:top;"><legend style='font-weight:bold;font-size: 24px;'>Total Bundles</legend><div class='ovAll' id="OverallPieces" style="font-size: 18px;padding:40px 0px;border-radius: 15px;box-sizing:border-box;color:white;text-shadow:#474747 2px 2px 4px;height:100px;width:200px"></div></div>
<div id="color-key" style='text-align: left;margin:15px 50px;display:inline-block;vertical-align:top;'>
	...
	<div style="height:24px;">
		<div style=";line-height: 90%;display:inline-block;height:20px;width:20px;border:1px black solid;background:#7C9FCE">
			<input type='button' name='c9510' onclick=' HCLpicker(event,this.value,this);' value="#7C9FCE" style="line-height: 0%;color:transparent;background:#7C9FCE;opacity:1;border:none;padding:0px;margin:0px;height:20px;width:20px;">
	...
	<div style="height:24px;">
		<div style=";line-height: 90%;display:inline-block;height:20px;width:20px;border:1px black solid;background:#FDBB9B">
			<input type='button' name='c7080' onclick=' HCLpicker(event,this.value,this);' value="#FDBB9B" style="line-height: 0%;color:transparent;background:#FDBB9B;opacity:1;border:none;padding:0px;margin:0px;height:20px;width:20px;">
		</div><span>Score 70&ndash;80%</span>
	</div>
	<div style="height:24px;">
		<div style=";line-height: 90%;display:inline-block;height:20px;width:20px;border:1px black solid;background:#EC9792">
			<input type='button' name='c0070' onclick=' HCLpicker(event,this.value,this);' value="#EC9792" style="line-height: 0%;color:transparent;background:#EC9792;opacity:1;border:none;padding:0px;margin:0px;height:20px;width:20px;">
...
Sort by:<select id='sortslct' class ="slctSmall">
		<option value = 2>Broken Bundles desc</option>
		<option value = 3>Percent desc</option> 
		<option value = -2>Alphabetical desc</option> 
		<option value = -3>Broken Bundles</option> 
...
<div id="chart-Area" style="display:inline-block;background:#f3f3f3;"></div>
<div id="chart-District" style="display:inline-block;background:#f3f3f3;"></div>
<div id="chart-Facility" style="display:inline-block;background:#f3f3f3;"></div>
</div>
<br>
<div style='min-width:99%;text-align:left;margin: 0 auto;'>
<div id="chart-FSFacility" style="display:inline-block;background:#f3f3f3;"></div>
<div id="chart-Device" style="display:inline-block;background:#f3f3f3;"></div>
<div id="chart-MClass" style="display:inline-block;background:#f3f3f3;"></div>
</div>
<br>
<div style='min-width:99%;text-align:left;margin: 0 auto;'>
<div id="chart-PKG" style="display:inline-block;background:#f3f3f3;"></div>
<div id="chart-DIS" style="display:inline-block;background:#f3f3f3;"></div>
<div id="chart-PREP" style="display:inline-block;background:#f3f3f3;"></div>
</div>
<br>
<div style='min-width:99%;text-align:left;margin: 0 auto;'>
<div id="chart-OWN" style="display:inline-block;background:#f3f3f3;"></div>
<div id="chart-MH" style="display:inline-block;background:#f3f3f3;"></div>
</div>
</div>
